...
// to NodeIDs using defaultNamespace when no namespace is embedded in the address.
//
// Implicit edges are injected for brew kinds: every HomeBrewPackages and
// HomeBrewCasks node automatically depends on all HomeBrewTaps nodes in the
// same namespace, so taps are always applied before packages without requiring
// users to write explicit dependsOn in their config.
//
// Returns an error if any address is invalid or a duplicate node ID is detected.
func Build(resources []resource.Resource, defaultNamespace string) (*DAG, error) {
	d := New()

	// Pass 1: collect HomeBrewTaps NodeIDs per namespace for implicit dep injection.
	tapsByNS := make(map[string][]NodeID)
	for _, res := range resources {
		...
		tapsByNS[ns] = append(tapsByNS[ns], ResourceNodeID(ns, res.GetKind(), meta.Name))
	}
	...
	for _, res := range resources {
		meta := res.GetMetadata()
		ns := meta.GetNamespace()
		id := ResourceNodeID(ns, res.GetKind(), meta.Name)

		deps := make([]NodeID, 0, len(meta.DependsOn))
		for _, addr := range meta.DependsOn {
			depID, err := ResolveAddress(addr, defaultNamespace)
			if err != nil {
		...
		if kind == resource.KindHomeBrewPackages || kind == resource.KindHomeBrewCasks {
			deps = withImplicitTapDeps(deps, tapsByNS[ns])
		}
